function [Zstar,obj] = algorithm(X, G, V, invVV, Z, Zstar, Y, N, k, lmd1, lmd2, lmd3, max_iter)
% N: totall numbers
% k : clusters
num_views = length(X);
    ...
    for kk = 1:N
        for qq = 1:N
            ...
            if r == 0
                ...
                for iv =1:r
                    sum_arZjp = sum_arZjp + alpha(iv)*rZ(iv);
                end

                if sum_arZjp > lmd3/(2*lmd2*r)
                    Zstar(kk,qq) = (2*lmd2*r*sum_arZjp - lmd3)/(2*lmd2*(r^2)+1e-10);
                elseif sum_arZjp < -lmd3/(2*lmd2*r)
                    Zstar(kk,qq) = (2*lmd2*r*sum_arZjp + lmd3)/(2*lmd2*(r^2)+1e-10);
                else
                    Zstar(kk,qq) = 0;
                end
