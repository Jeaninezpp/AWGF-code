...
resultdir = 'results/onecomplete/';
if (~exist('results/onecomplete','file'))
    mkdir('results/onecomplete');
    addpath(genpath('results/onecomplete/'));
end
%% Example Title
datadir = './setting/1-complete-data/';
dataname = {'100Leaves','bbcsport4vbigRnSp','buaaRnSp','caltech7','Mfeat','mfeatRnSp','ORL','orlRnSp'}; 
                ...
                for j = 1:length(lambda2) 
                    for d = 1:length(lambda3)
                        for repi = 1:1
                            ...
                            tic;
                            [Zstar, obj]  = algorithm_v1(X, G, V, invVV, Z_ini, Zstar_ini, truth, num_sample ,num_clusters, lambda1(i), lambda2(j), lambda3(d), max_iter);
                        ...
                        matname = [resultdir, char(dataname(datai)),'/Fold',num2str(folds),'/',num2str(lambda1(i)),'_',num2str(lambda2(j)),'_', num2str(lambda3(d)),'_v2.mat'];
