...
resultdir = 'results/randmiss/two_parameter/';
if (~exist('results/randmiss/two_parameter','file'))
    mkdir('results/randmiss/two_parameter');
    addpath(genpath('results/randmiss/two_parameter/'));
end
%% Example Title
datadir = './setting/random remove/';
dataname = {'100Leaves','buaaRnSp','caltech7','mfeatRnSp','ORL','orlRnSp','bbcsport4vbigRnSp'}; 
...
for datai = 1:4
    ...
    for per_in = 1:5  % per incomplete ratio 
        ...
        for folds = 1:1
            ...
            save([resultdir, char(dataname(datai)), '_',num2str(in_ratio),'%_.mat'], 'resultsmat');
        end % folds end
